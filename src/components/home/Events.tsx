      ...
      <div className="container relative z-10 px-4 mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-5 lg:gap-6">
          {homeEvents.map((event, index) => {
            const hasCta = Boolean(event.ctaHref && event.ctaLabel)
            ...
            return (
              ...
              >
                ...
                <ul className="space-y-2 text-[13px] md:text-[14px] text-[#3F4840]">
                ...
                <div className="mt-auto pt-5">
                  {hasCta ? (
                    <Link
                      href={event.ctaHref!}
                      className="inline-flex h-11 w-full items-center justify-center rounded-[4px] bg-[#78C86F] px-5 text-sm font-extrabold uppercase tracking-[0.04em] text-[#1A2518] hover:bg-[#88D57F] transition hover:scale-[1.02] transform duration-300"
                    >
                      {event.ctaLabel}
                    </Link>
                  ) : event.footerLabel ? (
                    <div className="border-t border-[#E2E7DC] pt-4">
                      <span className="inline-flex items-center rounded-full border border-[#CFE5C7] bg-[#F4FAEF] px-3.5 py-1.5 text-[11px] font-extrabold uppercase tracking-[0.08em] text-[#5B8D51]">
                        {event.footerLabel}
                      </span>
                    </div>
                  ) : null}
